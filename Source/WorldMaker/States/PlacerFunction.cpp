...
void PlacerFunction::mousePressed(int posx, int posy) {
    ...
    if (button != m_manager.buttonsEnd()) {
        m_pinned = *button;
        m_pinned.setPosition({0.f, 140.f});
        return;
    }
    //
    ...
    posx -= posx % 35;
    posy -= posy % 35;
    b.setPosition({static_cast<float>(posx), static_cast<float>(posy)});
    m_manager.addBlock(b);
}

void PlacerFunction::mouseMoved(int posx, int posy) {
    m_pinned.setPosition({static_cast<float>(posx), static_cast<float>(posy)});
}
